        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    return "menu"   # в браузере нельзя закрыть окно — возвращаем в меню
...
async def main():
    ...
    while True:
        ...
        if mode is None:
            break
        result = await Game(screen, clock, mode).run()
        if result == "quit":
            break
