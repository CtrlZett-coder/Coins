            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "Используй HTML-теги для оформления: <b>текст</b> для жирного. "
                        "Пиши развернутые, солидные отчеты. Добавляй секции, чтобы текст выглядел объемно. "
                        "В конце давай четкий совет по рынку."
                    )
                },
                {"role": "user", "content": f"Сделай подробный отчет по этим данным: {market_context}"}
            ]
        )
        # Убеждаемся, что ИИ не наставил лишних символов
        ai_text = response.choices[0].message.content.replace("**", "") 
    except:
        ai_text = "<b>⚠️ Ошибка связи с аналитическим центром.</b>\nРынки волатильны, следите за графиками в приложении!"
    
    builder = InlineKeyboardBuilder()
    builder.button(text="📊 Детали в приложении", web_app=types.WebAppInfo(url=BASE_URL))
    
    # КРИТИЧНО: используем parse_mode="HTML"
